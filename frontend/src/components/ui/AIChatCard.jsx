    ...
    return (
        <div className={cn("relative w-[360px] h-[460px] rounded-2xl overflow-hidden p-[2px]", className)}>
            ...
            <div className="relative flex flex-col w-full h-full rounded-xl border border-white/10 overflow-hidden bg-black/90 backdrop-blur-xl">
                ...
                {/* Header */}
                <div className="px-4 py-3 border-b border-white/10 relative z-10 flex items-center gap-2">
                    <Bot className="w-5 h-5 text-emerald-400" />
                    <h2 className="text-lg font-semibold text-white">AI Assistant</h2>
                </div>

                {/* Messages */}
                <div className="flex-1 px-4 py-3 overflow-y-auto space-y-3 text-sm flex flex-col relative z-10 scrollbar-none">
                    {messages.map((msg, i) => (
                        <motion.div
                            key={i}
                            ...
                            className={cn(
                                "px-3 py-2 rounded-xl max-w-[80%] shadow-md backdrop-blur-md flex items-start gap-2",
                                msg.sender === "ai"
                                    ? "bg-white/10 text-white self-start"
                                    : "bg-emerald-500/20 text-white border border-emerald-500/30 self-end"
                            )}
                        >
                            {msg.sender === "ai" && <Sparkles className="w-4 h-4 mt-1 text-emerald-400 shrink-0" />}
                            <span>{msg.text}</span>
                ...
                <div className="flex items-center gap-2 p-3 border-t border-white/10 relative z-10">
                    <input
                        className="flex-1 px-3 py-2 text-sm bg-black/50 rounded-lg border border-white/10 text-white focus:outline-none focus:ring-1 focus:ring-emerald-500/50 placeholder:text-gray-500"
                    ...
                    <button
                        onClick={handleSend}
                        className="p-2 rounded-lg bg-white/10 hover:bg-emerald-500/20 text-white hover:text-emerald-400 transition-all"
                    >
